translations = {
    'en': {
        'site': {
            'title': 'Sports Store',
            'rights': 'All rights reserved.'
        },
        'nav': {
            'home': 'Home',
            'cart': 'Cart',
            'favorites': 'Favorites',
            'login': 'Login',
            'register': 'Register',
            'logout': 'Logout',
            'toggle_theme': 'Toggle Theme',
            'search': 'Search',
            'search_placeholder': 'Search products...'
        },
        'auth': {
            'login': 'Login',
            'register': 'Register',
            'email': 'Email',
            'password': 'Password',
            'confirm_password': 'Confirm Password',
            'submit': 'Submit',
            'already_have_account': 'Already have an account? Login',
            'dont_have_account': "Don't have an account? Register",
            'login_title': 'Sign in to your account',
            'register_title': 'Create a new account',
            'username': 'Username',
            'remember_me': 'Remember me',
            'login_button': 'Sign in',
            'register_button': 'Register',
            'no_account': "Don't have an account?",
            'have_account': 'Already have an account?',
            'register_link': 'Register now',
            'login_link': 'Sign in',
            'logout_success': 'You have been logged out.',
            'login_success': 'Successfully logged in.',
            'register_success': 'Registration successful!'
        },
        'products': {
            'title': 'Our Products',
            'price': 'Price',
            'quantity': 'Quantity',
            'add_to_cart': 'Add to Cart',
            'out_of_stock': 'Out of Stock',
            'search': 'Search products...',
            'no_results': 'No products found',
            'contact_whatsapp': 'Contact on WhatsApp',
            'contact_telegram': 'Contact on Telegram',
            'categories': {
                'title': 'Categories',
                'all': 'All Products',
                'shoes': 'Shoes',
                'clothing': 'Clothing',
                'equipment': 'Equipment'
            },
            'items': {
                'sports_shoe_1': {
                    'name': 'Professional Sports Shoe',
                    'description': 'High quality sports shoe perfect for professional athletes'
                },
                'sports_shoe_2': {
                    'name': 'Elite Running Shoe',
                    'description': 'Professional running shoe designed for maximum performance'
                },
                'sports_shirt_1': {
                    'name': 'Performance Sports Shirt',
                    'description': 'Breathable sports shirt made with advanced moisture-wicking technology'
                },
                'Elite Pro Training Ball': {
                    'name': 'Elite Pro Training Ball',
                    'description': 'Professional-grade training ball with enhanced grip and durability.'
                },
                'Performance Basketball': {
                    'name': 'Performance Basketball',
                    'description': 'Competition-level basketball with superior bounce and control.'
                },
                'Advanced Tennis Racquet': {
                    'name': 'Advanced Tennis Racquet',
                    'description': 'Lightweight carbon fiber racquet for precise shots.'
                },
                'PowerFit Dumbbell Set': {
                    'name': 'PowerFit Dumbbell Set',
                    'description': 'Adjustable weight set with ergonomic grip design.'
                },
                'Competition Volleyball': {
                    'name': 'Competition Volleyball',
                    'description': 'Official match volleyball with enhanced durability.'
                },
                'Pro Resistance Kit': {
                    'name': 'Pro Resistance Kit',
                    'description': 'Complete resistance training set with multiple tension levels.'
                },
                'Premium Yoga Set': {
                    'name': 'Premium Yoga Set',
                    'description': 'High-density mat with alignment markers and accessories.'
                },
                'Champion Boxing Kit': {
                    'name': 'Champion Boxing Kit',
                    'description': 'Professional boxing gloves with hand wraps and protectors.'
                },
                'Speed Runner Elite': {
                    'name': 'Speed Runner Elite',
                    'description': 'Lightweight running shoes with advanced cushioning.'
                },
                'Aero Cycling Helmet': {
                    'name': 'Aero Cycling Helmet',
                    'description': 'Aerodynamic helmet with integrated ventilation system.'
                },
                'Pro Swim Goggles': {
                    'name': 'Pro Swim Goggles',
                    'description': 'Competition swimming goggles with anti-fog coating.'
                },
                'Tournament Badminton Set': {
                    'name': 'Tournament Badminton Set',
                    'description': 'Complete badminton set with premium racquets.'
                },
                'Master Table Tennis Kit': {
                    'name': 'Master Table Tennis Kit',
                    'description': 'Professional table tennis set with competition paddles.'
                },
                'Premium Golf Set': {
                    'name': 'Premium Golf Set',
                    'description': 'Complete golf club set with premium bag.'
                },
                'Speed Jump Rope Pro': {
                    'name': 'Speed Jump Rope Pro',
                    'description': 'Professional jump rope with adjustable length.'
                },
                'Mountain Bike Pro Helmet': {
                    'name': 'Mountain Bike Pro Helmet',
                    'description': 'Advanced protection helmet for mountain biking.'
                },
                'Climbing Safety Kit': {
                    'name': 'Climbing Safety Kit',
                    'description': 'Complete climbing gear set with harness and carabiners.'
                },
                'Pro Skateboard Complete': {
                    'name': 'Pro Skateboard Complete',
                    'description': 'Professional skateboard with custom artwork.'
                },
                'Yoga Wheel Pro': {
                    'name': 'Yoga Wheel Pro',
                    'description': 'Premium yoga wheel for advanced poses and stretching.'
                },
                'Training Boxing Bag': {
                    'name': 'Training Boxing Bag',
                    'description': 'Heavy-duty boxing bag with wall mount kit.'
                },
                'Trail Running Shoes': {
                    'name': 'Trail Running Shoes',
                    'description': 'All-terrain running shoes with enhanced grip.'
                },
                'Competition Swim Cap': {
                    'name': 'Competition Swim Cap',
                    'description': 'Professional silicone swim cap with ergonomic fit.'
                },
                'Elite Tennis Balls': {
                    'name': 'Elite Tennis Balls',
                    'description': 'Professional tennis balls for tournament play.'
                },
                'Basketball Training Set': {
                    'name': 'Basketball Training Set',
                    'description': 'Complete training kit with cones and agility ladder.'
                },
                'Soccer Goal Set': {
                    'name': 'Soccer Goal Set',
                    'description': 'Portable soccer goal with training net.'
                },
                'Premium Volleyball Net': {
                    'name': 'Premium Volleyball Net',
                    'description': 'Professional grade volleyball net with adjustable height.'
                },
                'Golf Practice Set': {
                    'name': 'Golf Practice Set',
                    'description': 'Indoor golf practice kit with mat and balls.'
                },
                'Table Tennis Robot': {
                    'name': 'Table Tennis Robot',
                    'description': 'Automatic table tennis training robot with ball recycling.'
                },
                'Badminton Pro Net': {
                    'name': 'Badminton Pro Net',
                    'description': 'Competition standard badminton net with quick setup.'
                },
                'Climbing Chalk Set': {
                    'name': 'Climbing Chalk Set',
                    'description': 'Premium climbing chalk with storage bag.'
                }
            }
        },
        'cart': {
            'title': 'Shopping Cart',
            'empty': 'Your cart is empty',
            'total': 'Total',
            'checkout': 'Proceed to Checkout',
            'update': 'Update Cart',
            'remove': 'Remove'
        },
        'favorites': {
            'title': 'My Favorites',
            'empty': 'You haven\'t added any favorites yet.',
            'browse': 'Browse Products'
        },
        'checkout': {
            'title': 'Checkout',
            'order_summary': 'Order Summary',
            'payment_info': 'Payment Information',
            'card_number': 'Card Number',
            'expiry': 'Expiry Date',
            'cvv': 'CVV',
            'place_order': 'Place Order',
            'total_amount': 'Total Amount'
        },
        'messages': {
            'login_success': 'Logged in successfully!',
            'register_success': 'Registration successful! Please login.',
            'logout_success': 'Logged out successfully!',
            'cart_updated': 'Cart updated successfully!',
            'item_added': 'Item added to cart!',
            'item_removed': 'Item removed from cart!',
            'favorite_added': 'Item added to favorites!',
            'favorite_removed': 'Item removed from favorites!',
            'order_success': 'Order placed successfully!',
            'order_error': 'An error occurred while placing your order.',
            'cart_empty': 'Your cart is empty.',
            'stock_error': 'Some items are no longer in stock.',
            'error': 'An error occurred. Please try again.'
        },
        'footer': {
            'about': 'About Us',
            'description': 'Your one-stop shop for quality sports equipment and accessories.',
            'contact': 'Contact Us',
            'email': 'Email: info@sportsstore.com',
            'phone': 'Phone: +93 771 749 100',
            'follow': 'Follow Us',
            'rights': 'All rights reserved.',
            'social': {
                'facebook': 'Follow us on Facebook',
                'twitter': 'Follow us on Twitter',
                'instagram': 'Follow us on Instagram'
            }
        },
        'search': {
            'placeholder': 'Search products...',
            'button': 'Search',
            'no_results': 'No results found',
            'results': 'Search results',
            'category': 'Category',
            'price': 'Price'
        }
    },
    'fa': {
        'site': {
            'title': 'فروشگاه ورزشی',
            'rights': 'تمامی حقوق محفوظ است.'
        },
        'nav': {
            'home': 'خانه',
            'cart': 'سبد خرید',
            'favorites': 'علاقه‌مندی‌ها',
            'login': 'ورود',
            'register': 'ثبت نام',
            'logout': 'خروج',
            'toggle_theme': 'تغییر تم',
            'search': 'جستجو',
            'search_placeholder': 'جستجوی محصولات...'
        },
        'auth': {
            'login': 'ورود',
            'register': 'ثبت نام',
            'email': 'آدرس ایمیل',
            'password': 'رمز عبور',
            'confirm_password': 'تکرار رمز عبور',
            'submit': 'ارسال',
            'already_have_account': 'حساب کاربری دارید؟ وارد شوید',
            'dont_have_account': 'حساب کاربری ندارید؟ ثبت نام کنید',
            'login_title': 'ورود به حساب کاربری',
            'register_title': 'ایجاد حساب کاربری جدید',
            'username': 'نام کاربری',
            'remember_me': 'مرا به خاطر بسپار',
            'login_button': 'ورود',
            'register_button': 'ثبت نام',
            'no_account': 'حساب کاربری ندارید؟',
            'have_account': 'قبلاً ثبت نام کرده‌اید؟',
            'register_link': 'ثبت نام کنید',
            'login_link': 'وارد شوید',
            'logout_success': 'با موفقیت خارج شدید.',
            'login_success': 'با موفقیت وارد شدید.',
            'register_success': 'ثبت نام با موفقیت انجام شد!'
        },
        'products': {
            'title': 'محصولات ما',
            'price': 'قیمت',
            'quantity': 'تعداد',
            'add_to_cart': 'افزودن به سبد خرید',
            'out_of_stock': 'ناموجود',
            'search': 'جستجوی محصولات...',
            'no_results': 'محصولی یافت نشد',
            'contact_whatsapp': 'تماس در واتساپ',
            'contact_telegram': 'تماس در تلگرام',
            'categories': {
                'title': 'دسته‌بندی‌ها',
                'all': 'همه محصولات',
                'shoes': 'کفش',
                'clothing': 'پوشاک',
                'equipment': 'تجهیزات'
            },
            'items': {
                'sports_shoe_1': {
                    'name': 'کفش ورزشی حرفه‌ای',
                    'description': 'کفش ورزشی با کیفیت بالا مناسب برای ورزشکاران حرفه‌ای'
                },
                'sports_shoe_2': {
                    'name': 'کفش دویدن حرفه‌ای',
                    'description': 'کفش دویدن حرفه‌ای طراحی شده برای عملکرد بهینه'
                },
                'sports_shirt_1': {
                    'name': 'تیشرت ورزشی حرفه‌ای',
                    'description': 'تیشرت ورزشی تنفس‌پذیر با تکنولوژی پیشرفته جذب رطوبت'
                },
                'Elite Pro Training Ball': {
                    'name': 'توپ تمرینی حرفه‌ای',
                    'description': 'توپ تمرینی با کیفیت حرفه‌ای با گریپ و دوام بالا'
                },
                'Performance Basketball': {
                    'name': 'توپ بسکتبال حرفه‌ای',
                    'description': 'توپ بسکتبال مسابقه‌ای با کنترل و پرش عالی'
                },
                'Advanced Tennis Racquet': {
                    'name': 'راکت تنیس پیشرفته',
                    'description': 'راکت تنیس سبک کربنی برای ضربات دقیق'
                },
                'PowerFit Dumbbell Set': {
                    'name': 'ست دمبل پاورفیت',
                    'description': 'ست وزنه قابل تنظیم با طراحی ارگونومیک'
                },
                'Competition Volleyball': {
                    'name': 'توپ والیبال مسابقه‌ای',
                    'description': 'توپ والیبال رسمی با دوام بالا'
                },
                'Pro Resistance Kit': {
                    'name': 'کیت مقاومتی حرفه‌ای',
                    'description': 'ست کامل تمرینات مقاومتی با سطوح مختلف کشش'
                },
                'Premium Yoga Set': {
                    'name': 'ست یوگا پریمیوم',
                    'description': 'مت یوگا با تراکم بالا و خطوط تراز به همراه لوازم جانبی'
                },
                'Champion Boxing Kit': {
                    'name': 'کیت بوکس قهرمانی',
                    'description': 'دستکش بوکس حرفه‌ای با بندها و محافظ‌های دست'
                },
                'Speed Runner Elite': {
                    'name': 'کفش دوی سرعت الیت',
                    'description': 'کفش دو سبک با کوسن پیشرفته'
                },
                'Aero Cycling Helmet': {
                    'name': 'کلاه دوچرخه‌سواری ایرو',
                    'description': 'کلاه آیرودینامیک با سیستم تهویه یکپارچه'
                },
                'Pro Swim Goggles': {
                    'name': 'عینک شنا حرفه‌ای',
                    'description': 'عینک شنای مسابقه‌ای با پوشش ضد بخار'
                },
                'Tournament Badminton Set': {
                    'name': 'ست بدمینتون مسابقه‌ای',
                    'description': 'ست کامل بدمینتون با راکت‌های پریمیوم'
                },
                'Master Table Tennis Kit': {
                    'name': 'کیت پینگ پنگ مستر',
                    'description': 'ست پینگ پنگ حرفه‌ای با راکت‌های مسابقه‌ای'
                },
                'Premium Golf Set': {
                    'name': 'ست گلف پریمیوم',
                    'description': 'ست کامل چوب‌های گلف با کیف پریمیوم'
                },
                'Speed Jump Rope Pro': {
                    'name': 'طناب ورزشی سرعتی حرفه‌ای',
                    'description': 'طناب ورزشی حرفه‌ای با طول قابل تنظیم'
                },
                'Mountain Bike Pro Helmet': {
                    'name': 'کلاه دوچرخه‌سواری کوهستان حرفه‌ای',
                    'description': 'کلاه محافظ پیشرفته برای دوچرخه‌سواری کوهستان'
                },
                'Climbing Safety Kit': {
                    'name': 'کیت ایمنی کوهنوردی',
                    'description': 'ست کامل تجهیزات کوهنوردی با هارنس و کارابین'
                },
                'Pro Skateboard Complete': {
                    'name': 'اسکیت‌برد حرفه‌ای کامل',
                    'description': 'اسکیت‌برد حرفه‌ای با طراحی سفارشی'
                },
                'Yoga Wheel Pro': {
                    'name': 'چرخ یوگا حرفه‌ای',
                    'description': 'چرخ یوگای پریمیوم برای حرکات پیشرفته و کششی'
                },
                'Training Boxing Bag': {
                    'name': 'کیسه بوکس تمرینی',
                    'description': 'کیسه بوکس با دوام بالا به همراه کیت نصب دیواری'
                },
                'Trail Running Shoes': {
                    'name': 'کفش دوی کوهستان',
                    'description': 'کفش دو با گریپ قوی برای همه زمین‌ها'
                },
                'Competition Swim Cap': {
                    'name': 'کلاه شنای مسابقه‌ای',
                    'description': 'کلاه شنای سیلیکونی حرفه‌ای با طراحی ارگونومیک'
                },
                'Elite Tennis Balls': {
                    'name': 'توپ تنیس حرفه‌ای',
                    'description': 'توپ تنیس مخصوص مسابقات'
                },
                'Basketball Training Set': {
                    'name': 'ست تمرینی بسکتبال',
                    'description': 'ست کامل تمرینی با مخروط و نردبان چابکی'
                },
                'Soccer Goal Set': {
                    'name': 'ست دروازه فوتبال',
                    'description': 'دروازه فوتبال قابل حمل با تور تمرینی'
                },
                'Premium Volleyball Net': {
                    'name': 'تور والیبال پریمیوم',
                    'description': 'تور والیبال حرفه‌ای با ارتفاع قابل تنظیم'
                },
                'Golf Practice Set': {
                    'name': 'ست تمرینی گلف',
                    'description': 'ست تمرین گلف داخل سالن با مت و توپ'
                },
                'Table Tennis Robot': {
                    'name': 'ربات پینگ پنگ',
                    'description': 'ربات تمرینی پینگ پنگ با سیستم بازیافت توپ'
                },
                'Badminton Pro Net': {
                    'name': 'تور بدمینتون حرفه‌ای',
                    'description': 'تور بدمینتون استاندارد مسابقات با نصب سریع'
                },
                'Climbing Chalk Set': {
                    'name': 'ست پودر سنگنوردی',
                    'description': 'پودر سنگنوردی با کیفیت به همراه کیف نگهداری'
                }
            }
        },
        'cart': {
            'title': 'سبد خرید',
            'empty': 'سبد خرید شما خالی است',
            'total': 'مجموع',
            'checkout': 'ادامه خرید',
            'update': 'به‌روزرسانی سبد',
            'remove': 'حذف'
        },
        'favorites': {
            'title': 'علاقه‌مندی‌های من',
            'empty': 'هنوز محصولی به علاقه‌مندی‌ها اضافه نکرده‌اید.',
            'browse': 'مشاهده محصولات'
        },
        'checkout': {
            'title': 'تکمیل خرید',
            'order_summary': 'خلاصه سفارش',
            'payment_info': 'اطلاعات پرداخت',
            'card_number': 'شماره کارت',
            'expiry': 'تاریخ انقضا',
            'cvv': 'CVV',
            'place_order': 'ثبت سفارش',
            'total_amount': 'مبلغ کل'
        },
        'messages': {
            'login_success': 'با موفقیت وارد شدید!',
            'register_success': 'ثبت نام با موفقیت انجام شد! لطفا وارد شوید.',
            'logout_success': 'با موفقیت خارج شدید!',
            'cart_updated': 'سبد خرید به‌روز شد!',
            'item_added': 'محصول به سبد خرید اضافه شد!',
            'item_removed': 'محصول از سبد خرید حذف شد!',
            'favorite_added': 'محصول به علاقه‌مندی‌ها اضافه شد!',
            'favorite_removed': 'محصول از علاقه‌مندی‌ها حذف شد!',
            'order_success': 'سفارش با موفقیت ثبت شد!',
            'order_error': 'خطایی در ثبت سفارش رخ داد.',
            'cart_empty': 'سبد خرید شما خالی است.',
            'stock_error': 'برخی از محصولات دیگر موجود نیستند.',
            'error': 'خطایی رخ داد. لطفا دوباره تلاش کنید.'
        },
        'footer': {
            'about': 'درباره ما',
            'description': 'فروشگاه کامل تجهیزات و لوازم ورزشی با کیفیت',
            'contact': 'تماس با ما',
            'email': 'ایمیل: info@sportsstore.com',
            'phone': 'تلفن: ۱۰۰ ۷۴۹ ۷۷۱ ۹۳+',
            'follow': 'ما را دنبال کنید',
            'rights': 'تمامی حقوق محفوظ است.',
            'social': {
                'facebook': 'ما را در فیسبوک دنبال کنید',
                'twitter': 'ما را در توییتر دنبال کنید',
                'instagram': 'ما را در اینستاگرام دنبال کنید'
            }
        },
        'search': {
            'placeholder': 'جستجوی محصولات...',
            'button': 'جستجو',
            'no_results': 'نتیجه‌ای یافت نشد',
            'results': 'نتایج جستجو',
            'category': 'دسته‌بندی',
            'price': 'قیمت'
        }
    }
} 